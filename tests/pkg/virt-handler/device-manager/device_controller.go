...
func (c *DeviceController) NodeHasDevice(devicePath string) bool {
	...
	log.Log.Infof("=============no from node has device: %s", devicePath)
	return (err == nil)
}
...
func (c *DeviceController) updatePermittedHostDevicePlugins() []Device {
	...
	if len(hostDevs.USBDevices) != 0 {
		...
		for _, usbDev := range hostDevs.USBDevices {
			log.Log.Infof("Permitted USB device in the cluster, ID: %s, resourceName: %s",
				usbDev.VendorSelector, usbDev.ResourceName)
			supportedUSBDeviceMap[strings.ToLower(usbDev.VendorSelector)] = usbDev.ResourceName
		}
		...
		log.Log.Info("==============================device controller :usb device======================")
	}
	log.Log.Info("==============================virt-handler/device-manager/device_controller.go :pcihostdevice=====================")
	...
	if len(hostDevs.PciHostDevices) != 0 {
		...
		for _, pciDev := range hostDevs.PciHostDevices {
			...
			log.Log.Infof("========%s", pciDev.ResourceName)
		}
...
func (c *DeviceController) Run(stop chan struct{}) error {
	logger := log.DefaultLogger()
	logger.Info("===========================Device manager controller run======================")
