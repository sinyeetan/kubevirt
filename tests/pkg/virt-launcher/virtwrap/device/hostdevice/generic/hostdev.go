...
func CreateHostDevices(vmiHostDevices []v1.HostDevice) ([]api.HostDevice, error) {
	return CreateHostDevicesFromPools(vmiHostDevices, NewPCIAddressPool(vmiHostDevices), NewMDEVAddressPool(vmiHostDevices), NewUSBAddressPool(vmiHostDevices))
}

// flagXY
func CreateHostDevicesFromPools(vmiHostDevices []v1.HostDevice, pciAddressPool, mdevAddressPool hostdevice.AddressPooler, usbAddressPool hostdevice.AddressPooler) ([]api.HostDevice, error) {
	...
	for _, dm := range hostDevicesMetaData {
		log.Log.Info("===========virt-launcher/virtwrap/device/hostdevice/generic/hostdev.go createfrompool=================")
		log.Log.Infof(dm.ResourceName)
	}
	...
	pciHostDevices = append(pciHostDevices, usbHostDevices...)
