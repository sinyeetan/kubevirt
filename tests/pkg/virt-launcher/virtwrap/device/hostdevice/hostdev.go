...
// flagXY
func createUSBHostDevice(hostDeviceData HostDeviceMetaData, hostAddress string) (*api.HostDevice, error) {
	hostAddr := strings.Split(hostAddress, ":")
	vendor := api.Vendor{Id: hostAddr[0]}
	product := api.Product{Id: hostAddr[1]}
	addr := api.Address{Bus: "0", Type: "usb"}

	domainHostDevice := &api.HostDevice{
		Alias:   api.NewUserDefinedAlias(hostDeviceData.AliasPrefix + hostDeviceData.Name),
		Source:  api.HostDeviceSource{Vendor: &vendor, Product: &product},
		Address: &addr,
		Type:    "usb",
		Managed: "yes",
	}
